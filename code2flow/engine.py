...
def get_sources_and_language(raw_source_paths, language):
    ...
    for source in sorted(raw_source_paths):
        logging.info("Processing files from %r.", source)
...
def make_file_group(tree, filename, extension, show_path=False):
    """
...
def map_it(
    ...
    for source, file_ast_tree in file_ast_trees:
        file_group = make_file_group(file_ast_tree, source, extension, show_path)
    ...
    # logging.info("Found node %r.", node.namespace_ownership())
    ...
    logging.info("Found nodes %r." % sorted(n.token for n in all_nodes))
    logging.info(
        "Found calls %r."
...
def code2flow(
    ...
    exclude_functions.append("(global)")
...
def main(sys_argv=None):
    ...
    parser.add_argument("--show-path", action="store_true", help="show the full path")
